...
import { FilterCurrSaleExDecDto } from 'src/app/DTOs/Sale/FilterCurrSaleExDecDto';
import { CurrencySalesService } from 'src/app/Services/currency-sales.service';

@Component({
  selector: 'app-sale-exdec-list-by-currencysale-id',
  templateUrl: './sale-exdec-list-by-currencysale-id.component.html',
  styleUrls: ['./sale-exdec-list-by-currencysale-id.component.scss']
})
export class SaleExdecListByCurrencysaleIdComponent implements OnInit {
  ...
  getCurrSaleExDecList() {
    this.isLoadingData = true;
    this.currencySalesService.getFilteredCurrExdecByCurrSaleId(this.filterExDecSales).subscribe((result) => {
  ...
  setNavigate(filterParam: string = null, value: any = null) {
    ...
    this.router.navigate(['sales/sale-exdec-list-currSaleId'], {
      queryParams: {
        pageId: pageid,
